...
| 任务交互输入不可用                      | 确认模板和任务已启用交互输入、任务仍为 RUNNING、Agent 为 ONLINE 且 hello 声明 `task-input-v1`；不要通过重发旧消息绕过能力或租约检查。 |
| 输入发送超时或通道关闭                  | 记录 taskId、inputId、时间和安全错误码，确认命令仍在运行；Server 不自动重发，用户确认后使用新的 inputId 重试。                        |
...
交互输入正文永不进入 Server/Agent 日志、审计、数据库或断线缓冲。敏感输入也不得出现在截图和工单中；
Agent 只在当前任务内存中暂存脱敏值，并在写 stdin 之前更新 stdout/stderr 脱敏器。能力仅支持 HTTPS/WSS
下的单行标准输入，等待输入不会暂停任务超时，单实例控制权不会在 Server 重启或多 Server 实例之间共享。
