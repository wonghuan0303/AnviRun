...
## Windows 与 Linux 内网运行

Windows x64 可使用 `deploy/windows/package-agent.ps1` 生成 ZIP 和 SHA-256，解压后通过
`start-agent.ps1 -ConfigPath <build-agent.toml> -AgentExecutable <build-agent.exe>` 启动。
Linux x86_64 可通过 `deploy/linux/build.sh` 从源码构建，并通过
`start-agent.sh <build-agent.toml> [agent-executable]` 启动。两端也都提供 `start-all` 入口，供
Server 与 Agent 同机运行。当前流程不安装 Windows Service 或 systemd unit；Token 只放在权限
受限的配置文件中。完整说明见 `docs/windows-deployment.md` 和 `docs/linux-deployment.md`。
