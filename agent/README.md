...
当前 T4.3 已实现 platform.config.json、跨平台构建命令和 `RUNNING`/`UPLOADING`/`FAILED` 生命周期上报；T5.1 已实现有界磁盘日志缓冲、连续序号、Server ACK、同进程短暂断线回放、文件持久化和浏览器订阅；T5.3 已实现 artifactDir 根层 ZIP 扫描、清单 ACK、流式上传和 `SUCCEEDED` 闭环；T6.1 已补充短断线恢复、终态 ACK 和任务创建幂等。T6.2～T6.4 已按 Windows 可信内网精简范围实现；第一版真实 Server + Rust Agent + PostgreSQL 验收入口为 `deploy/windows/release-check.ps1`。
