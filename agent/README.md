...
- 命令使用 Windows `cmd.exe /D /S /C` 或 Unix `/bin/sh -lc`，stdout/stderr 以有界分片发送 `task.log`；成功发送 `RUNNING`、`UPLOADING`，仅扫描 artifactDir 根层大小写不敏感的 `.zip` 文件并上传，随后发送 `task.completed`，失败发送带安全原因和可选退出码的 `task.failed`。
...
当前 T4.3 已实现 platform.config.json、跨平台构建命令和 `RUNNING`/`UPLOADING`/`FAILED` 生命周期上报；T5.1 已实现有界磁盘日志缓冲、连续序号、Server ACK、同进程短暂断线回放、文件持久化和浏览器订阅；T5.3 已实现 artifactDir 根层 ZIP 扫描、清单 ACK、流式上传和 `SUCCEEDED` 闭环；T6.1 已补充短断线恢复、终态 ACK 和任务创建幂等。Windows 与 Linux 均提供内网前台部署入口；第一版真实 Server + Rust Agent + PostgreSQL 验收入口仍为 `deploy/windows/release-check.ps1`。
...
Linux 当前支持通过 `deploy/linux/build.sh` 从源码构建 release Agent，并通过
`deploy/linux/start-agent.sh` 单独启动，或通过 `deploy/linux/start-all.sh` 与 Server 一同
启动。当前不生成 Linux 安装包、不安装 systemd unit；配置、目录权限和升级流程见
